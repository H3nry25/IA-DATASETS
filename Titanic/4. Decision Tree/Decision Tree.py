...
# Nota: Los árboles de decisión NO necesitan que los datos estén normalizados, 
# pero como ya los traemos limpios y escalados del archivo 2, los usamos así sin problema.

print("--- 3. ENTRENAMIENTO DEL ÁRBOL DE DECISIÓN ---")
# Limitamos la profundidad a 4 (max_depth=4) para que el gráfico del árbol sea legible y no gigante
...
plt.title('Matriz de Confusión - Decision Tree (Titanic)')
...
fig, ax = plt.subplots(figsize=(24, 12)) # Tamaño ajustado para pantallas normales
plot_tree(model, 
...
}).sort_values(by='Importancia', ascending=True) # Ascending true para que el gráfico quede de mayor a menor

print("\nImportancia de las variables:")
print(feature_importances.sort_values(by='Importancia', ascending=False))

plt.figure(figsize=(10, 6))
plt.barh(feature_importances['Variable'], feature_importances['Importancia'], color='forestgreen')
...
# --- GUARDAR MODELO ---
ruta_modelo = os.path.join(directorio_actual, 'decision_tree_titanic.pkl')
joblib.dump(model, ruta_modelo)
print(f"\nModelo guardado exitosamente como '{ruta_modelo}'")

print("\n--- 5. EXPLICABILIDAD CON SHAP ---")
# Crear un explainer de SHAP para el árbol
explainer = shap.TreeExplainer(model)
shap_values = explainer.shap_values(X_test)

# En los modelos de clasificación binaria, shap_values suele ser una lista de 2 elementos
# Usamos el índice 1 que corresponde a la clase positiva (Sobrevivió)
